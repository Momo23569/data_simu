...
class MYDATA(Dataset):
    ...
    def dfprepare_FS(self):
        ...
        # D = pd.read_csv("Code/data/FourSquare/NYC/dataset_TSMC2014_NYC.csv")
        D = pd.read_csv("Code/data/FourSquare/TKY/dataset_TSMC2014_TKY.csv")
        # Time
    ...
    def attrprepare(self):
        ...
        if not self.EXIST:
            ...
            # 检查和创建路径，然后保存数据
            data_path = self.PATH + 'DATA_' + self.LOCATION_MODE + '.npy'
            check_and_create_path(data_path)
            np.save(data_path, self.DATA)

            gps_path = self.PATH + 'GPS_' + self.LOCATION_MODE + '.npy'
            check_and_create_path(gps_path)
            np.save(gps_path, self.GPS)

            if self.NAME != 'GeoLife':
                poi_path = self.PATH + 'POI_' + self.LOCATION_MODE + '.npy'
                check_and_create_path(poi_path)
                np.save(poi_path, self.POI)
